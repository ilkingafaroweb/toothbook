...
export const InsuranceSelector: React.FC = () => {
    ...
    // Handle selecting main insurance card
    const handleSelectCard = (id: number) => {
        setSelectedCard(prevSelectedCard => (prevSelectedCard === id ? null : id));

        // Show the input field and fetch other insurance companies if "Other" is selected
        if (id === -1) {
            setIsInputVisible(true);
            // Fetch additional insurance options
    ...
    useEffect(() => {
        if (response && isInputVisible) {
            setOtherInsurances(response.otherInsuranceCompanies);
        }
    }, [response, isInputVisible]);
